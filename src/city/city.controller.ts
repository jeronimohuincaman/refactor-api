...
import { UserPaginator } from '@/users/user.paginator.dto'; // J: Importo el paginator de usuario
...
export class CityController
  ...
  @Post()
  async create(@Body() data: CreateCityDto, @Owner() usuario: UserPaginator /* J: Especifico el tipo  */) {
    return this.city.create({
      data: {
        id_visible: (await this.city.count()) + 1,
        ...data,
        uploadUserID: usuario.id.toString(), // J: Convertir a string
      },
      include: {
        uploadUser: {
  ...
  @Get()
  async findAll(@Query() /* // J: Doy un nombre descriptivo al parametro */ city: CityPaginatorDto) {
    ...
    // J: Cambié el nombre de la constante para que sea más descriptivo
